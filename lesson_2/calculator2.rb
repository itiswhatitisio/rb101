  # ask the user for two numbers
  # ask the user for an operation to perform
  # perform the operation on the two numbers
  # output the result

  # answer = Kernel.gets()
  # Kernel.puts(answer)

  # Bonus features 4 and 5
  # Create a configuration file with all the messages

  require 'yaml'
  MESSAGES = YAML.load_file('messages.yml')

  LANGUAGE = 'de'

  def messages(message, lang='en')
    MESSAGES[lang][message]
  end

  def prompt(key)
    message = messages(key, LANGUAGE)   # make sure the "messages" method is declared above this line)
    Kernel.puts("=> #{message}")
  end

  def valid_number?(num)
    num.to_i() != 0
  end

  def operation_to_message(op)
    case op
    when '1'
      'Adding'
    when '2'
      'Subtracting'
    when '3'
      'Multiplying'
    when '4'
      'Dividing'
    end
  end

  prompt('welcome')

  username = ''

  loop do
    username = Kernel.gets().chomp()

    if username.empty?()
      prompt('valid_name')
    else
      break
    end
  end

prompt("Hi #{username}!")

loop do # main loop
  number1 = ''
  number2 = ''

  loop do
    prompt('first_num')
    number1 = Kernel.gets().chomp() # gets() returns a string

    if valid_number?(number1)
      break
    else
      prompt('valid_num')
    end
  end

  loop do
    prompt('second_num')
    number2 = Kernel.gets().chomp() # gets() returns a string

    if valid_number?(number2)
      break
    else
      prompt('valid_num')
    end
  end

  operator_prompt = <<-MSG 
    "What operation would you like to perform? 
    1) add 
    2) subtract 
    3) multiply 
    4) divide
  MSG

  prompt(operator_prompt)

  operator = ''
  loop do
    operator = Kernel.gets().chomp()

    if %w(1 2 3 4).include?(operator)
      break
    else
      prompt('valid_operator')
    end
  end

  # if expressions can return a value
  # refactor to case statement

  prompt("#{operation_to_message(operator)} the two numbers...")

  result = case operator
    when '1' # Local variables initialized within an if can be accessed outside of the if.
      number1.to_i() + number2.to_i()
    when '2' # pay attention to the equality operator!
      number1.to_i() - number2.to_i()
    when '3'
      number1.to_i() * number2.to_i()
    when '4'
      number1.to_f() / number2.to_f() # without the to_f() method, the output will be an integer
    end

  prompt("The result is #{result}")

  prompt('another_cal')
  answer = Kernel.gets().chomp()
  break unless answer.downcase().start_with?('y')

end

prompt('final_message')